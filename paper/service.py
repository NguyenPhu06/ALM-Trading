...
class PaperTradingService:
    ...
    def enter(self,request:PaperOrderRequest,*,quote,setup_status,risk_decision,data_quality,provider_status,prediction,volatility="NORMAL_VOLATILITY",news_risk="LOW",reasons=(),market_context=None,mtf_context=None,liquidity_context=None,indicator_context=None):
        ...
        position=PaperPosition.open(request.symbol,request.direction,order.executed_price,request.quantity,request.timestamp,request.stop_loss,request.take_profit);order=replace(order,position_id=position.position_id);self.positions[position.position_id]=position;self.orders.append(order)
    ...
    def mark(self,position_id,price,timestamp):
        ...
        return pnl
    def dca(self,position_id,request,*,quote,market_regime,structure_state,risk_state,strategy_valid=True,htf_valid=True):
        position=self.positions[position_id];gate=self.risk.evaluate(new_entry=False,exposure=position.notional,quantity=request.quantity,dca_entries=position.dca_entries,data_quality="VALID",provider_status="ONLINE",model_valid=True,strategy_valid=strategy_valid and htf_valid)
        if not gate.allowed:return PaperExecutionResult(False,None,gate.rejection_reasons[0],0.,reason_codes=tuple(f"WHY_REJECTED:{r}" for r in gate.rejection_reasons))
        result=self.execution.execute(request,quote=quote);order=result.order
        if order:
            position.add(order.executed_price,order.quantity,request.timestamp);self.orders.append(order);event=DCAEvent(uuid4().hex,position_id,position.dca_entries,request.timestamp,order.executed_price,order.quantity,"WHY_DCA:CONTEXT_VALID",market_regime,structure_state,risk_state);self.dca_events.append(event)
    ...
    def close_position(self,position_id,*,price,timestamp,reason=("WHY_EXIT:MANUAL_PAPER_COMMAND",)):
        position=self.positions[position_id];gross=position.mark(price,timestamp);commission=sum(o.commission for o in self.orders if o.position_id==position_id);slippage=sum(o.slippage*o.quantity for o in self.orders if o.position_id==position_id);net=self.account.realize(gross,commission,slippage,timestamp);position.realized_pnl=net;position.state=PositionState.CLOSED;self.positions.pop(position_id);journal=next((j for j in self.journals if j.trade_id==position_id),None)
        if journal:self.journals[self.journals.index(journal)]=replace(journal,dca_history=tuple(asdict(e) for e in self.dca_events if e.position_id==position_id),exit_reason=tuple(reason),final_result={"gross_pnl":gross,"commission":commission,"slippage":slippage,"net_pnl":net})
        if self.repository:self.repository.save_position(position);self.repository.save_account(self.account);self.repository.save_equity(self.account);self.repository.save_journal(self.journals[-1])
        return position

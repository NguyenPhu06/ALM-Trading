...
class GlobalKillSwitch:
    ...
    def permits(self,*,new_entry:bool):return not(self.enabled and new_entry)
...
class PaperRiskEngine:
    ...
    def evaluate(self,*,new_entry=True,drawdown=0.,exposure=0.,quantity=0.,dca_entries=0,daily_drawdown=0.,concurrent_positions=0,volatility="NORMAL_VOLATILITY",spread=0.,news_risk="LOW",data_quality="VALID",provider_status="ONLINE",model_valid=True,strategy_valid=True)->PaperRiskDecision:
        r=[]
        if not self.kill_switch.permits(new_entry=new_entry):r.append("GLOBAL_KILL_SWITCH")
        ...
        if provider_status in {"OFFLINE","DEGRADED"}:r.append("PROVIDER_UNAVAILABLE")

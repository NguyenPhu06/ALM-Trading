...
export async function loadDashboard(symbol:string){const paths=['/dashboard/overview',`/dashboard/market/${symbol}`,`/dashboard/mtf/${symbol}`,`/dashboard/liquidity/${symbol}`,`/dashboard/indicators/${symbol}`,`/dashboard/ai/${symbol}`,`/dashboard/strategy/${symbol}`,'/dashboard/risk','/dashboard/positions','/dashboard/performance','/dashboard/journal','/dashboard/alerts',`/dashboard/timeline/${symbol}`,'/dashboard/mt5'];const values=await Promise.all(paths.map(get));return Object.fromEntries(paths.map((p,i)=>[p.split('/')[2]||'overview',values[i]]))}
